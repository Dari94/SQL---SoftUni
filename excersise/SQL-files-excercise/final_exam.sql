...
CONSTRAINT fk_players_skills_teams
...
(
SELECT
pl.first_name AS first_name, pl.last_name AS last_name,
 pl.salary * 2 AS salary,char_length(pl.first_name) AS coach_level
FROM players AS pl
WHERE age >= 45
);
SELECT
...
/*3.UPDATE*/
UPDATE coaches AS c11
JOIN
(SELECT c.id, c.first_name, c.coach_level, COUNT(pc.player_id) AS count_players FROM coaches AS c
JOIN players_coaches AS pc
ON c.id = pc.coach_id
WHERE c.first_name LIKE 'A%'
GRoup BY pc.coach_id
HAVING count_players > 0) AS c22
ON c11.id = c22.id
SET c11.coach_level = c11.coach_level + 1;
...
SELECT * FROM players;
SELECT pl.id, concat(first_name,' ',pl.last_name) AS full_name, pl.age, pl.position, pl.hire_date
FROM players AS pl
...
SELECT t.`name`, t.established, t.fan_base, count(pl.id) AS players_count
FROM teams AS t
...
SELECT max(skt.speed) AS max_speed, skt.`name` AS town_name
FROM (
        SELECT sk.speed, t.`name` 
        FROM towns AS t
...
/*09.	Total salaries and players by country*/
     ...
     SELECT cpl.`name` AS `name`, count(cpl.id) AS total_count_of_players,
     SUM(cpl.salary) AS total_sum_of_salaries
     FROM (
     SELECT  c.`name` ,pl.id,  pl.salary
...
DELIMITER $$                                             
CREATE FUNCTION udf_stadium_players_count4(stadium_name VARCHAR(30))   
RETURNS INTEGER
DETERMINISTIC
BEGIN
...
JOIN countries AS c
ON c.id = t.country_id
WHERE st.`name` = 'Linklinks';

SELECT udf_stadium_players_count4('Linklinks') as `count`;


/*11.	Find good playmaker by teams*/
DELIMITER $$
CREATE PROCEDURE udp_find_playmaker2( min_dribble_points INT ,team_name VARCHAR(45))
BEGIN
SELECT concat(pl.first_name,' ',pl.last_name) AS full_name, pl.age,
pl.salary, sk.dribbling, sk.speed, te.`name`
...
END $$
CALL udp_find_playmaker2(20, 'Skyble');
...
pl.salary, sk.dribbling, sk.speed, te.`name`
 ...
 JOIN teams AS te
 ON te.id = pl.team_id
WHERE sk.dribbling > 20
AND te.`name` = 'Skyble'
ORDER BY sk.speed desc
LIMIT 1;

DELIMITER $$
CREATE FUNCTION udf_stadium_players_count7(stadium_name VARCHAR(30))   
RETURNS INTEGER
DETERMINISTIC
BEGIN
   DECLARE result INTEGER;
...
SELECT udf_stadium_players_count4('Jaxworks') as `count`;